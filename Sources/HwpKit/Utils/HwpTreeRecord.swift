...
class HwpTreeRecord {
    ...
    let level: UInt32
    let payload: Data
    var children: [HwpTreeRecord]

    init(tagId: UInt32, level: UInt32, payload: Data) {
        self.tagId = tagId
        self.level = level
        self.payload = payload
        self.children = [HwpTreeRecord]()
    }
}

func parseTreeRecord(data: Data) -> HwpTreeRecord {
    var reader = DataReader(data)
    let root = HwpTreeRecord(tagId: 0, level: 0, payload: Data())
    ...
    while !reader.isEOF() {
        ...
        parent.children.append(HwpTreeRecord(tagId: tagId, level: level, payload: payload))
    }
