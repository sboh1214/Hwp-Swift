...
public struct HwpBinData: HwpFromData {
    ...
    init(_ reader: inout DataReader) throws {
        ...
        if property.type == .link {
            ...
            absolutePath = reader.read(WCHAR.self, absolutePathLength).string
            let relativePathLength = reader.read(WORD.self)
            relativePath = reader.read(WCHAR.self, relativePathLength).string
        } else {
            streamId = reader.read(UInt16.self)
            let extensionLength = reader.read(WORD.self)
            extensionName = reader.read(WCHAR.self, extensionLength).string
        }
    }
}
